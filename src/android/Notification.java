...
public class Notification extends CordovaPlugin {
     ...
     * @param callbackContext   The callback context.
     */
    public synchronized void prompt(final String message, final String title, final JSONArray buttonLabels, final String defaultValue, final String placeholder,  final CallbackContext callbackContext) {
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                ...
                promptInput.setHint(placeholder);
                promptInput.setText(defaultValue);
                AlertDialog.Builder dlg = createDialog(cordova); // new AlertDialog.Builder(cordova.getActivity(), AlertDialog.THEME_DEVICE_DEFAULT_LIGHT);
                dlg.setMessage(message);
                dlg.setTitle(title);
                dlg.setCancelable(true);
                            ...
                            new AlertDialog.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    try {
                                        ...
                                        result.put("input1", promptInput.getText().toString().trim().length()==0 ? defaultValue : promptInput.getText());                      
